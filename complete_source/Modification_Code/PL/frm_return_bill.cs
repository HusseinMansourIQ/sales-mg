...
namespace Sales_Management_2.PL
{
    ...
    public partial class frm_return_bill : Form
    {
        // لاعادة البينات الى الفورم الاول عند اغلاق الفورم الثاني
        private static frm_return_bill frm;
        static void Frm_formclosed(object sender, FormClosedEventArgs e)
        {
            frm = null;
        } // لاعادة البينات الى الفورم الاول عند اغلاق الفورم الثاني
        public static frm_return_bill get_return_bill_Form   // لاعادة البينات الى الفورم الاول عند اغلاق الفورم الثاني
        {
            get
            {
                if (frm == null)
                {
                    frm = new frm_return_bill();
 ...
 txt_prodName.Text = this.dgv_sellingPill.CurrentRow.Cells[0].Value.ToString();
        ...
        private void btn_SaveOnly_Click(object sender, EventArgs e)
        {
            ...
            else if (txt_PaidMony.Text == "" || txt_PaidMony.Text == string.Empty) { MessageBox.Show("الرجاء قم بادخال المبلغ المدفوع الى الزبون "); }
            else
            {
                try
                {
                    decimal restmony = (Convert.ToDecimal(txt_PaidMony.Text) - Convert.ToDecimal(txt_totalAfterDis.Text));
                    //احضار اخر قائمة+1 
                    ...
                    sales.AddSalesBill(max_SalesBill_Id, dateTimePicker1.Value, x, cust_id, txt_totalBeforDis.Text,
                                     ...
                                     txt_RestMony.Text,Convert.ToDecimal(new_balance), Program.U_ArabicName,"ارجاع مواد","");
        ...
        private void txt_PaidMony_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if ((e.KeyCode == Keys.Enter || e.KeyCode == Keys.Tab) && txt_PaidMony.Text != string.Empty)
            {
                ...
                {
btn_SaveOnly.Focus();
                txt_RestMony.Text = (Convert.ToDouble(txt_PaidMony.Text) - Convert.ToDouble(txt_totalAfterDis.Text)).ToString();
        ...
        {
            PL.frm_bill_management frm_b_mng = new frm_bill_management();
            frm_b_mng.Show();
        ...
        {
            txt_total_Dis.Text = string.Format("{0:#,##0}", double.Parse(txt_total_Dis.Text));
        }
    }
}
